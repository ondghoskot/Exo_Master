...
<div class="muscle-groups">
  <h1>Select a Muscle</h1>
  <h2>Arms</h2>
  <input type="checkbox" class="biceps muscles-helper" :checked="true" id="biceps"> <label for="biceps">Biceps</label>
...
<div class="exercise__list">
      <h2 class="exercise__title">{{bodyPart}} exercises:</h2>
      <ul>
...
export default {
  ...
  async mounted() {
    await this.fetchExerciseByBodyPart("upper arms");
    this.setupSvgInteractions();
  },
  methods: {
    async fetchExerciseByBodyPart(bodyPart) {
      try {
        const response = await this.$axios.get(`https://exercisedb.p.rapidapi.com/exercises/bodyPart/${bodyPart}`, {
          headers: {
        ...
        if (response.status === 200) {
          this.bodyPart = bodyPart.charAt(0).toUpperCase() + bodyPart.slice(1);
    ...
    setupSvgInteractions() {
      ...
      groups.forEach((group) => {
        group.addEventListener("mouseover", this.handleMouseOver);
        group.addEventListener("mouseout", this.handleMouseOut);
        group.addEventListener("click", this.handleClick);
      });
    },
    handleMouseOver(event) {
      const id = this.getElementId(event);
    ...
    uncheckAllInputs(){
      let inputs = document.getElementsByClassName('muscles-helper');
    ...
    async handleClick(event) {
      ...
      if (input) {
        ...
        const apiBodyPart = this.bodyPartMapping[id];
        if (apiBodyPart) {
          await this.fetchExerciseByBodyPart(apiBodyPart);
        } else {
          console.error("No mapping found for body part:", id);
        }
      }
    },
    getElementId(event) {
      // Use closest to find the nearest 'g' with an 'id'
      const targetGroup = event.target.closest("g[id]");
      return targetGroup ? targetGroup.id.toLowerCase() : null;
    },
  },
};
</script>
...
.muscle-groups {
  ...
  svg g[id] path {
    opacity: 1;  // Lighten the muscle color by increasing opacity
    transition: opacity .25s ease-in-out;
    stroke: $white;
    stroke-width: 0.5px; 
    fill: lighten($white, 100%);  // Lighten the fill color
  }

  svg g g[id]:hover path {
    cursor: pointer;
    opacity: .6;  // Further lightened on hover
    fill: lighten($color-1, 20%) !important;  // Lighten the hover color as well
  }
...
.exercise{
  ...
  &__list{
    ...
    box-shadow: 1px 1px 20px 3px rgba(210, 10, 46, 0.7);
    border-radius: 12px;
    min-width:32rem;
    width:32rem;
